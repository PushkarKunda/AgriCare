#!/usr/bin/env python3
"""
Robust Deep WMS Discovery Script for 2025-26 Soil Health Data in SPSR Nellore.
...
LAYER_2025_26 = "28_515_shc_2025-26"
OUTPUT_FILE = "nellore_soil_data_2025-26.csv"
...
def gather_target_points():
    ...
    # 1. Existing reference CSV files (search local and parent directory)
    candidate_files = [
    ...
    # 2. Dense Grid over Nellore district (14.05 to 15.35 lat, 79.30 to 80.25 lon)
    tile_step = 0.005  # ~550m
    ...
    # Deduplicate approximate coordinates
    unique_points = []
    seen = set()
...
def main():
    ...
    # Pre-populate with existing records if present
    discovered_features = {}  # fid -> dict
    ...
    print(f"Starting robust WMS parallel scan for 2025-26 layer across {len(bbox_list)} targets...")
    ...
    with sync_playwright() as p:
        ...
        chunk_size = 25  # 25 concurrent requests per batch with 5s timeout
        ...
        for i in range(0, len(bbox_list), chunk_size):
            ...
            try:
                ...
                if chunk_idx % 20 == 0 or chunk_idx == total_chunks:
                    print(f"[{chunk_idx}/{total_chunks}] Total 2025-26 Features: {len(discovered_features)} (+{new_found} new in this batch)")

                    # Save checkpoint with only desired columns
    ...
    print(f"\n[SUCCESS] Completed discovery! Total 2025-26 soil records: {len(final_df)}")
